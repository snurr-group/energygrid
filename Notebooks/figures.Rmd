...
p_sub_isotherm <- 
  ...
  annotate("segment", x = 95, xend = 95, y = 4.1, yend = 51.1, color = 2, arrow = double_arrow) +
  annotate("segment", x = 85, xend = 85, y = 27.4, yend = 47.4, color = 4, arrow = double_arrow) +
  annotate("text", 92, 4.1, label = "47 g/L\nBetter deliverable capacity", color = 2, hjust = 1, vjust = 0) +
  annotate("text", 82, 27.4, label = "20 g/L\nSteep isotherm", color = 4, hjust = 1, vjust = 0)
...
p_sub_energies <- 
  ...
  ggplot(aes(loc, dens, color = id)) + geom_point() +
...
plot_grid(
  ...
  p_beta_diff_t + theme(legend.position=c(0, 0.2), legend.direction="horizontal"),
  ncol=1,
  #align = "v",
  labels=default_subfig_labels, label_size=12
  ) %>%
...
zeo_model <- 
  ...
  fit_textural_model()
expect_equal(nrow(zeo_model$orig_x), 1000)
hmof_t_test_data <- 
  hmof_textural_data %>% 
...
ccdc_top500 %>% filter(!(id %in% old_screening) & !(id %in% old2_screening)) %>% arrange(id) %>% select(id) %>% write_tsv("BigData/Output/new_ccdc.txt", col_names=FALSE)  # only 11 in the top 750 in round three, none of which are in the top 600.  Get around to this if there's any remaining in the top 500 in the final model.
...
screening_gcmc <- 
  list(orig="ScreenCSD", extended="ExtraCSD2", fill3="ExtraCSD3") %>% 
...
ccdc_top500 %>% 
  ...
  select(id) %>%
  write_tsv("BigData/Output/missing_500_ccdc.txt", col_names=FALSE)
...
```{r ccdc_t_swing}
# Try a temperature swing (77 K -> 160 K) in addition to the pressure swing (100 bar -> 2 bar)
...
if (!exists("ccdc_160k_predicted")) {
  # If we use 160 K grids, we'll have to retrain the hMOF model on 160 K grids, too
  # Train the hMOF model.  Based on the p_beta_diff_t figure.
  trained_160k_mod <- run_bin_model_spec(
    p_2bar_sets$training,
    mutate(p_2bar_data, g.L = va100bar - h2.g.L.2.160),
    default_binspec
    ) %>% 
    .$fitted_model %>% 
    .[[1]]
...
ccdc_desorb_strategy %>% 
  filter(y_160k_pred < 100 & y_160k_pred > 0) %>% 
  ggplot(aes(y_160k_pred)) +
  geom_histogram()

# Ranking plot
#ccdc_desorb_strategy %>% 
#  ggplot(aes(r_77k, r_160k)) +
#  geom_point() +
#  parity_line +
...
screening_3d %>%
  ...
  save_fig_table_with_colnames("ccdc_screening.tex", digits=3)
...
# 160 K P/T swing predictions (but still 2 bar, not 5 bar)
ad_hoc_h2_grids %>% 
  pred_grid(trained_160k_mod, ., default_binspec)
...
# Note: nsample_h2_db will have to be saved later below, once the tex exporter function is defined.
...
p_geom_distr <- 
  ...
  bind_rows(hMOF = ., ToBaCCo = select(tob_textural, LCD, PLD, vf, density, gsa, vsa), .id = "db") %>% 
...
(list(
  ...
  map(~ eval_test_grid(trained_mod, .x, default_binspec, mutate(p_2bar_data, y_act = g.L))) %>% 
...
# Save the number of samples justification from up above, to determine the binspec in the first place.
nsample_h2_db %>% 
  mutate(Bin=paste(lower, "to", upper, "kJ/mol")) %>% 
  mutate(Bin=ifelse(is.na(lower), "(Intercept)", Bin)) %>% 
...
coarse_hmof_model <- run_model_on_partitions(hmof_hist_sets, hmof_y_to_join, coarse_binspec, db_name = "hMOFs", align_bins = "downward")
print(coarse_hmof_model)
tobacco_data %>% 
  mutate(y_act = fh.h2.g.L.100.77 - fh.h2.g.L.2.77) %>% 
  eval_test_grid(coarse_hmof_model$trained_mod, tob_hist_sets$testing, coarse_binspec, ., db_name = "ToBaCCo", align_bins="downward")
coef_tbl(coarse_hmof_model$trained_mod$mod) %>%
  left_join(bin_loc_from_spec(coarse_binspec, align_bins="downward"), by="bin")
```
...
fine_h2_grids <- read_rds("BigData/Robj/fine_h2.Rds")
...
fine_mof_5_comparison %>% 
  ...
  #mutate(bin_e = paste(lower, "to", upper, "kJ/mol")) %>% 
  ggplot(aes(bin, metric, fill=spacing)) +
...
list(
  ...
  map_dfr(
    ~ pred_grid(trained_mod, .x, default_binspec),
    .id = "spacing"
    )
...
fine_diff_df
fine_diff_df %$% parity_plot(coarse, fine)
fine_diff_df %$% hist(coarse - fine)
...
fine_diff_df %>% 
  mutate(coarse_err = coarse - y_act, fine_err = fine - y_act) %>% 
...
fine_h2_grids %>% 
  ...
  ggplot(aes(num_pts, coarse - fine)) +  # Granted, num_pts will be 1000x as high as earlier
  geom_point()

# TODO: ADD INTERMEDIATE 0.5 AA grid RESULTS HERE
...
### General
* Clear remaining notes in the manuscript
* Grid convergence testing!!!  Test on ~100 structures, then maybe with 0.5 AA spacing as an intermediate.  Would it be interesting/worthwhile to train on a 0.5 AA set?
  * Easy simulation: see if there's a difference between 0.5 AA and 0.1 AA predictions.  That could give good confidence in the method.
* Import/train 5 bar, 160 K model
  * TODO: PUT THIS UP ABOVE SOMEWHERE IN THE CCDC SECTION, THEN MAKE THE PARITY PLOT/PREDICTIONS.  SAME MOFs?
* Check the new parameters for the bottom bin in default_binspec.  Does it use enough samples?  How is it (and the model type) justified?  Fit a new "highly simplified model" for the text in the manuscript.
  * Update the "simple" equation in the manuscript.
  * This should just be a binspec of 7.5 kJ/mol, and the bin_below parameter.
  * Also need to extract the parity plot (for SI) which shows that the fit is worse, but still not too bad.
* Set up CV error bars, etc., from the manuscript.
* Check the beta robustness figure (see bottom).  Also regenerate it.

### More simulations/grids (older notes)
* Check that the CCDC MOF simulations are complete (original 77 K might only be 497/500?)
  * Also see if we need to run new simulations for 5 bar, 160 K conditions
* Run calculations of the top CCDC candidates and confirm results. (get from mateo:ScreenCSD, ExtraCSD2, and ExtraCSD3)
  * Delegate snapshots to Arun
  * Update main text in the paper
  * Comment on a few non-physical structures
  * SI template: add textural properties and try to set up in a separate SI document
* Use 160 K grids (and model) to see if top CCDC candidates are the same as a pure 77 K pressure swing
  * Run top CCDC candidates based on the T-swing model
  * It looks like there ~500 3D structures predicted with a deliverable capacity over 50 g/L once you add in temperature swing, so those seem the most exciting.  To study those, it's mostly just calculating the 2 bar 160 K data, plus any 100 bar MOFs that are missing.
* Discuss CCDC scripts with Scotty
  * "When I get back I will think about rerunning the solvent script without the monodentate flag. Once I remember how to use the CCDC API it won't take that long and then we can decide to either use them or just ignore it. As you say, it is not "wrong" to use the script as we did so long as that is made clear in the paper. And there probably would be cases where there are monodentate solvents that should be removed and are not. It's a two way street and solvent removal is difficult."
  * "acknowledging the difficulty of automating structure processing in the manuscript"
* Check out this proton diffusion NEB/grid paper? https://arxiv.org/pdf/1710.03468.pdf

### Figures
* Defer textural property predictions to a follow-up paper, likely with Maciek's help

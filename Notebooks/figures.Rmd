...
p_beta_diff_p <- 
  ...
  #plot_cat_betas(., default_binspec)
...
p_beta_diff_t <- 
  ...
  #overlay_cat_betas(base_hist, ., default_binspec)
  plot_cat_betas(., default_binspec) %>% 
  overlay_hist_bins(base_grid, default_binspec)
# Generate the plots
plot_grid(
  p_beta_orig,
...
screening_gcmc <- 
  list(orig="ScreenCSD", extended="ExtraCSD2", fill3="ExtraCSD3", quest_mixed="../../quest/FillCCDC77K") %>% 
...
p_mfu_isotherm <- 
  ...
  geom_text(
    data = tibble(T=mfu_expt_temperatures, `P bar`=90, mol.kg=c(37, 23, 2), source=NA),
    aes(label=T)
    ) +
  scale_y_continuous(limits=c(0,52), sec.axis = sec_axis(~.*mmol.g_to_g.L, name="Uptake (g/L)", breaks=10*(0:6))) +
  scale_shape(NULL) + scale_color_discrete(guide=FALSE)
...
plot_grid(
  ...
  save_ben_fig("mfu_fig.png", nrow = 2, base_aspect_ratio = 2.0)  # 1.5 might work better here, if we crop the mfu_viz figure

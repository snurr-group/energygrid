...
# Instead of collecting more data to draw this figure, let's just fit a Type I Langmuir isotherm to sketch it for the outline
p_isotherm_fit <- 
  p_concept_isotherms %>%
  group_by(id) %>% nest() %>%
  # We need to fit on an inverted langmuir model to get K^-1 in the numerator with our constant
  # P = x / (K*(1-x))
  mutate(model = map(data, ~ lm(.x, formula = bar ~ g.L / (1 - g.L) + 0))) %>%
  mutate(k = map_dbl(model, ~ 1 / coef(.x))) %>% 
  #select(-model) %>% unnest()  # Can get back a data.frame instead of the next part with the fit
  select(id, k)
# Instead of using this, let's use the fits from Arun, saved in the same directory as the source data
cartoon_pressures <- c(seq(0.05, 0.95, 0.05), 1:100)
double_arrow <- arrow(ends="both", type="closed", length=unit(2, "mm"))
p_sub_isotherm <- 
  # p_isotherm_fit %>% 
  #deframe() %>%  # tibble function to convert two-column df to a named list
  #map_dfr(~ data_frame(bar = 1:100, k = .x), .id = "id") %>% 
  #mutate(g.L = 50 * k * bar / (1 + k * bar)) %>% 
  list(
    `hMOF-1898` = c(3.16783, 0.04234),
    `hMOF-5072244` = c(62.03316, 1.15935)
  ) %>% 
  #expand.grid(bar = cartoon_pressures, id = c("hMOF-1898", "hMOF-5072244")) %>% 
  ...
  #annotate("segment", x = 25, xend = 25, y = 4.1, yend = 51.1, color = 4, arrow = double_arrow) +
  #annotate("segment", x = 90, xend = 90, y = 27.4, yend = 47.4, color = 2, arrow = double_arrow) +
  annotate("segment", x = 90, xend = 90, y = 5.6, yend = 60.6, color = 2, arrow = double_arrow) +
  #annotate("segment", x = 25, xend = 25, y = 36.3, yend = 55.7, color = 4, arrow = double_arrow) +
  annotate("segment", x = 15, xend = 15, y = 36.3, yend = 53, color = 4, arrow = double_arrow) +
  annotate("text", 90, 45, label = "47 g/L", color = 2, hjust = 1.1) +
  annotate("text", 15, 45, label = "20 g/L", color = 4, hjust = -0.1) +
  annotate("text", 10, 60, label = "Steep isotherm", color = 4, hjust = 0) +
  annotate("text", 50, 20, label = "Better deliverable capacity", color = 2)
...
### General
* Add z-scored results to the SI
* See remaining notes in the manuscript
* Add a few sentences about the fully robust 4-term beta model.  Maybe also the small number of samples (consider a plot of accuracy versus number of samples for the SI)
...
* Run calculations of the top CCDC candidates and confirm results. (get from mateo:ScreenCSD, ExtraCSD2, and ExtraCSD3)
  ...
  * Set up SI template for the CCDC analysis (write out textural data, histograms, and automated import of figures.  Then we just need to import the snapshot png's, etc.)
  * Potential columns for template: CCDC refcode, reference paper?, CIF viz, energy histogram, y_act, y_pred, textural properties (VF, LCD, PLD, ASA)
* Get refined GCMC for case study from Arun

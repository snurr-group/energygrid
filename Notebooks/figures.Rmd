...
p_160k_testing <- 
  ...
  eval_test_grid(trained_160k_mod, p_2bar_sets$testing, default_binspec, .)
p_160k_testing
plot_grid(
...
p_160k_xy_hmof <- 
  (
    p_160k_5bar_data %>% 
    mutate(dc.5bar.160k = va100bar - fh.h2.g.L.5.160, dc.2bar.77k = va100bar - va2bar) %$% 
    parity_plot(dc.2bar.77k, dc.5bar.160k)
    ) %>% 
  #annotate_plot("2500 hMOFs") %>% 
  label_77k_vs_160k()

plot_grid(
  p_160k_xy_hmof %>% annotate_plot("2500 hMOFs"),
  p_160k_xy_hmof %>% annotate_plot("1000 CCDC MOFs") %>% make_draft("TODO"),
  p_160k_xy_hmof %>% annotate_plot("Top CCDC MOFs") %>% make_draft("TODO"),
  ncol=3,
  labels=default_subfig_labels, label_size=12
  ) %>%
...
* Check the beta robustness figure (see bottom).  Also regenerate it.  Need to adjust the bin_below to fix one of the gaps.
...
* See if there's any CCDC 100 bar or 5 bar data remaining to calculate to check out these top MOFs.

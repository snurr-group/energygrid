...
trained_mod <- mixed_partitioned_mod$trained_mod  # Note: the graphs below aren't yet consistent
...
plot_grid(
  p_beta_orig,
  p_beta_diff_p + theme(legend.position=c(0.2, 0.2), legend.direction="horizontal"),
  p_beta_diff_t + theme(legend.position=c(0.2, 0.2), legend.direction="horizontal"),
  ncol=1,
  #align = "v",
  labels=default_subfig_labels, label_size=12
  ) %>%
...
hyper_tuned_labels <- 
  ...
  mutate(labelval = format(labelval, digits=3)) %>% 
  mutate(labelval = ifelse(near(binwidth, 2.5+0.10), paste(labeltype, "=", labelval), labelval)) %>% 
...
p_robust <- 
  ...
  #ylim(c(-12.5, 2.5))
  geom_text(data=hyper_tuned_labels, aes(x=binwidth, y=lower, label=labelval), color="purple", size=3, hjust=1)  # add training data/fit to the plot
...
p_mfu_isotherm <- 
  ...
  ggplot(aes(`P bar`, mol.kg, color=T, shape=source)) +
  # geom_line() +
  geom_point() +
  ...
  geom_text(
    data = tibble(T=c("77 K", "160 K", "298 K"), `P bar`=90, mol.kg=c(37, 23, 2), source=NA),
    aes(label=T)
    ) +
  scale_y_continuous(sec.axis = sec_axis(~.*mmol.g_to_g.L, name="Uptake (g/L)"))
...
plot_grid(
  ...
  save_ben_fig("mfu_fig.png", nrow = 2, base_aspect_ratio = 2.0)
...
plot_grid(
  ...
  p_tob_from_hmof$plots$resid_normality,
  plot_resid_vs_act(hmof_partitioned_mod$pred_df),
  plot_resid_vs_act(p_tob_from_hmof$pred_df),
  ncol=2, nrow=2,
  labels=default_subfig_labels, label_size=12
  ) %>% 
...
less_trained_stats %>% 
  ...
  save_fig_table_with_colnames("small_n_stats.tex", digits=4)
...
tobacco_data %>%
  mutate(y_act = fh.h2.g.L.100.77 - fh.h2.g.L.2.77) %>%
  eval_test_grid(trained_mod, tob_hist_sets$testing, default_binspec, ., db_name = "ToBaCCo MOFs\n(mixed model)") %>%
  .$pred_df %>% ggplot(aes(y_act, y_act-y_pred, color=lcd)) + geom_point()

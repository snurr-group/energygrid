...
p_tob_mod <- run_model_on_partitions(tob_hist_sets, tob_y_to_join, default_binspec, 0, db_name = "ToBaCCo MOFs")
p_tob_mod
...
p_ch4_vol <- p_2bar_data %>% 
  ...
  run_model_on_partitions(p_ch4_sets, ., ch4_binspec, 0, plot_units="cm\u00B3/cm\u00B3", db_name = "hMOFs")
...
p_beta_ch4 <- 
  ...
  map(function(x) x$trained_mod$mod) %>% 
  map_dfr(coef_tbl, .id="cat") %>% 
  overlay_cat_betas(ch4_base_hist, ., ch4_binspec, scaling = 10000.0)
...
ccdc_predicted %>% arrange(desc(y_pred)) %>% top_n(750, y_pred) %>% filter(!(id %in% old_screening)) %>% arrange(id) %>% select(id) %>% write_tsv("BigData/Output/new_ccdc.txt", col_names=FALSE)  # only ~250
...
p_beta_diff_p$layers[[2]]$data %>% 
  mutate(beta = beta * 1000.0) %>%  # undo scaling from `overlay_cat_betas`
...
p_beta_ch4$layers[[2]]$data %>% 
  mutate(beta = beta * 10000.0) %>%  # undo scaling from `overlay_cat_betas`
  select(lower, upper, cat, beta) %>% 
  spread(cat, beta) %>% 
...
* Consider adding a geom_text type label to the hMOF vs. ToBaCCo betas in the SI.

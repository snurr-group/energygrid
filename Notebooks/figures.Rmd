...
base_hist <-
  ...
  filter(id == "hMOF-0") %>% 
  plot_hist_bins(default_binspec)
p_beta_orig <- base_hist %>% overlay_cat_betas(coef_tbl(hmof_partitioned_mod$trained_mod$mod), default_binspec)
p_beta_orig
  ...
  plot_grid(
    ...
    p_tob_from_hmof$pred_df %>%  
      ...
      annotate_plot(paste0(
        ...
        "MAE = ", format(p_tob_from_hmof$testing_fit["MAE"], digits=2), " ", "g/L", "\n",
        "RMSE = ", format(p_tob_from_hmof$testing_fit["RMSE"], digits=2), " ", "g/L"
        ), "bottom.right"),
...
p_coef_compare <- 
  list(`hMOF` = hmof_partitioned_mod$trained_mod$mod, `ToBaCCo` = p_tob_mod$trained_mod$mod) %>% 
  map_dfr(coef_tbl, .id="cat") %>%
  overlay_cat_betas(base_hist, ., default_binspec) +

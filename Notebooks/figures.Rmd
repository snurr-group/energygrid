...
```
# This works!  Just need to make the lines thicker or the plot different
hmof_h2_grid %>% stepped_hist_spec(default_binspec) %>% mutate(height = metric) %>% inner_join(color_from_binloc(bin_loc_from_spec(default_binspec)), by="bin") -> test_h2

test_h2 %>% group_by(bin, loc, color) %>% summarize(mh = mean(height)) %>% ungroup %>% ggplot(aes(loc, mh)) + geom_col(aes(fill = I(color))) +  # need I() so ggplot doesn't think the rgb strings are a factor
    guides(fill = FALSE) +
    labs(
      x = "Energy (kJ/mol)",
      y = NULL
    ...
    geom_violin(data=test_h2, aes(group = loc, y=height), scale="width", alpha=0, color="green")
    # Scale so the max widths are the same

...
tob_t_test_data <- 
  ...
  select(id, y_act, LCD, PLD, vf, density, gsa, vsa)
eval_textural_model(tob_t_model, tob_t_test_data)
eval_textural_model(zeo_model, tob_t_test_data)
eval_textural_model(tob_t_model, hmof_t_test_data)  # Note, only the ToBaCCo model works on the hMOFs, not vice-versa
...
* Finish work on textural properties!  Particularly ToBaCCo might be interesting.  Does the model transfer to the CCDC dataset?
  * Answer from preliminary data: hMOF doesn't transfer to ToBaCCo, though ToBaCCo transfers to the hMOFs
  * Clean up analysis for next week's meeting.
  * Think about how to write this up, if we report it.
  * Are there any (simple) published hMOF models we can validate on the hMOFs and try on ToBaCCo?  That would confirm this behavior.

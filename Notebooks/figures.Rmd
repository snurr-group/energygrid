...
list(50, 100, 500, 1000) %>% 
  map(function(mapx) {
    ...
    (function(x, nsample) {
      ...
      x$training <- 
        training_data %>% 
        filter(id %in% sample(training_ids, nsample))
      x
      })(., nsample=mapx) %>% 
...
### General
* See remaining notes in the manuscript
* Maybe also the small number of samples (consider a plot of accuracy versus number of samples for the SI)
...
* Run calculations of the top CCDC candidates and confirm results. (get from mateo:ScreenCSD, ExtraCSD2, and ExtraCSD3)
  * Delegate snapshots to Arun next week
...
* Use 160 K grids (and model) to see if top CCDC candidates are the same as a pure 77 K pressure swing
  * If the results are interesting, possibly also try at 300 K?
  * Run top CCDC candidates based on the T-swing model
  * Reran GCMC on the top CCDC candidate for Omar
  * The predictions look highly sensitive to the lowest bin, so be sure to implement Neda's binning suggestion before running more GCMC on the "top" CCDC structures

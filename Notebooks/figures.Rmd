...
screening_gcmc <- read_tsv(
  "BigData/Mateo/EnergyGrid/ScreenCSD/volume.tsv",
  skip=1,
...
# Some quick testing.  To organize if it's useful.
# Can we train a good model on the CCDC data?
ccdc_h2_grids %>% filter(id %in% ccdc_gcmc$id) %>% run_bin_model(ccdc_gcmc, default_binspec["step"], default_binspec["width"], default_binspec[c("from", "to")], alpha=0, lambda=NULL) %>% .$fitted_model %>% .[[1]] %>% eval_test_grid(., filter(ccdc_h2_grids, id %in% screening_gcmc$id), default_binspec, mutate(screening_gcmc, y_act=g.L))
# Turns out the CCDC model can also predict hMOFs?
ccdc_h2_grids %>% filter(id %in% ccdc_gcmc$id) %>% run_bin_model(ccdc_gcmc, default_binspec["step"], default_binspec["width"], default_binspec[c("from", "to")], alpha=0, lambda=NULL) %>% .$fitted_model %>% .[[1]] %>% eval_test_grid(., hmof_hist_sets$testing, default_binspec, mutate(gcmc_data, y_act=g.L))
ccdc_h2_grids %>% filter(id %in% ccdc_gcmc$id) %>% run_bin_model(ccdc_gcmc, default_binspec["step"], default_binspec["width"], default_binspec[c("from", "to")], alpha=0, lambda=NULL) %>% .$fitted_model %>% .[[1]] %>% eval_test_grid(., tob_hist_sets$testing, default_binspec, mutate(tobacco_data, y_act=fh.h2.g.L.100.77-fh.h2.g.L.2.77))

# The fits are moderately transferable to the hMOF and ToBaCCo data.
# Is the earlier problem caused by z-scoring, the y-intercept (average pore), both, or something else?
# Dimensionality might still play a role, but this graph is indicating that the pure metals might not be the sole problem.
...
# Also verified that there weren't line ending issues (see mateo:EnergyGrids/TestCRLF) by checking a few
# input files and testing that RASPA reads in the unit cells correctly both ways.  The grid files
# are identical whether or not there's a CRLF line ending in the UnitCells line.


# Now let's check the MOF atom dimensionality from Zeo++.
...
ccdc_parity_for_dimensionality <- 
  ccdc_h2_grids %>%
  filter(id %in% screening_gcmc$id) %>%
  eval_test_grid(trained_mod, ., default_binspec, mutate(screening_gcmc, y_act=g.L)) %>%
  .$pred_df %>% 
  left_join(ccdc_dimensionality, by="id") %>% 
  mutate(has_loose_molec = (molecules > 0)) %>% 
  mutate(has_periodic_framework = (frameworks > 0)) %>% 
  ggplot(aes(y_act, y_pred)) +
  geom_point() +
  parity_line
ccdc_parity_for_dimensionality + facet_grid(has_periodic_framework~has_loose_molec)
ccdc_parity_for_dimensionality + facet_grid(f3d~f1d)
# From this analysis, it doesn't look like dimensionality is the answer


# Maybe the problem is grid point sampling convergence?
ccdc_h2_grids %>% group_by(id) %>% summarize(hi = sum(counts)) %>% mutate(hi=ifelse(hi>15^3, 15^3, hi)) %>% .$hi %>% hist

ccdc_h2_grids %>%
  group_by(id) %>% summarize(num_pts = sum(counts)) %>% ungroup() %>% 
  right_join(ccdc_parity_for_dimensionality$data, by="id") %>% 
  #mutate(num_pts = ifelse(num_pts > 15^3, 15^3, num_pts)) %>% 
  mutate(num_pts = (num_pts > 15^3)) %>% 
  mutate(frameworks = ifelse(frameworks > 2, 3, frameworks)) %>% 
  ggplot(aes(y_act, y_pred, color = num_pts)) +
  #ggplot(aes(y_act, y_pred)) +
  geom_point() +
  coord_fixed() +
  parity_line +
  facet_grid(num_pts~frameworks)

# Let's also check the chemistry
ccdc_formulas <- read_rds("BigData/Robj/csd_formulas.Rds")
(ccdc_formulas %>%
  select(cif, str_formula, C, H, O, N, P, S) %>% 
  rename(id = cif) %>% 
  right_join(ccdc_parity_for_dimensionality$data, by="id") %>% 
  mutate(dimensions = paste(f1d, f2d, f3d, sep="/")) %>% 
  ggplot(aes(y_act, y_pred, composition=str_formula, dimensionality=dimensions, cif=id)) +
  geom_point() + coord_fixed() + parity_line
  ) %>% ggplotly

# Maybe it's overpredicting the weakly attractive bin? (Not enough granularity to indicate sufficient binding?)
# For example, BAQSOB has a terrible prediction of 29.5 g/L for a prediction of 52 g/L
ccdc_h2_grids %>% filter(id=="BAQSOB_stripped") %>% plot_hist_bins(default_binspec, y_title="Fraction")
ccdc_h2_grids %>% 
  filter(id %in% screening_gcmc$id) %>% 
  ...
  select(id, metric) %>% 
  inner_join(ccdc_parity_for_dimensionality$data, by="id") %>% 
  ggplot(aes(y_act, y_pred, color = metric)) +
  geom_point() + coord_fixed() + parity_line +
  scale_color_viridis(option = "B")  # "inferno"
```
...
# What happens if we try to use H2 grids to predict CH4 adsorption?
# \textit{WIP.  I'm not sure what to do with this figure, but I thought the experiment might be interesting for understanding.} Can we use the hydrogen grid to predict methane adsorption?  How much of our model is capturing specific energetic interactions versus pore characterization like textural properties?  These parity plots were trained and tested on the hMOF database using the hydrogen grids for the explanatory variable but methane deliverable capacity for the response.  \textbf{If this is interesting, a big question would be trying to transfer it to ToBaCCo}
crossover_ch4_gcmc <-
  p_2bar_data %>% 
  mutate(g.L = ch4.cm3.cm3.65.298 - ch4.cm3.cm3.5_8.298)
crossover_h2_sets_for_ch4 <- partition_data_subsets(hmof_h2_grid, crossover_ch4_gcmc, DATA_SPLIT)
crossover_part_mod <- run_model_on_partitions(crossover_h2_sets_for_ch4, crossover_ch4_gcmc, default_binspec, 0, plot_units="cm\u00B3/cm\u00B3")
crossover_part_mod$plots %$%
  plot_grid(
    parity_training %>% rescale_ch4_parity(),
    parity_testing %>% rescale_ch4_parity(),
    ncol = 2,
  ...
  save_ben_fig("h2_crossover_to_ch4.png", nrow = 1, ncol = 2)
```
...
### General
* Finish work on textural properties!  Particularly ToBaCCo might be interesting
* Regenerate Figure 1 from isotherm-a-b-20180112/isotherm-data.txt
* Fix figure 2, etc.
* Model prediction on an empty box
* Incorporate "coef_p.tex", etc., into the SI.
* Adjust scaling factor for betas.  Possibly remove +/- red/blue color scheme for simplicity.

### Poor CCDC predictions
* If our model is solely based on LJ interactions, shouldn't the non-CNPS structures perform just as well?
* Is it something with pore size distribution?  Maybe we're extrapolating too far beyond the textural property assumptions of the training set.
* Perhaps the attraction between -1 and 0 kJ/mol is not granular enough for the "point frameworks."
* Should resolve this discrepancy before publication.
...
* Run calculations of the top CCDC candidates and confirm results. (get from mateo:ScreenCSD)
* Grid convergence testing!!!

### z-scoring
* Figure out beta plots without normalization.  Set `zscore=FALSE` as default for `fit_glmnet`
  * It's tricky because there's advantages to both ways: see [Wikipedia article on standardized coefficients](https://en.wikipedia.org/wiki/Standardized_coefficient).
  * [Minitab](http://blog.minitab.com/blog/adventures-in-statistics-2/when-is-it-crucial-to-standardize-the-variables-in-a-regression-model) and another [tutorial](http://www.listendata.com/2017/04/how-to-standardize-variable-in-regression.html) have more to say.  The model $R^2$, etc., are identical both ways.  It's just the model terms that are different.
  * The ISLR book also recommends preprocessing the variables with z-scoring before ridge regression to avoid problems with beta.  But, IIRC `glmnet` implicitly handles that step (and transforms the coefficients and variables back for the final fitted model)
  * Maybe the slightly attractive region is causing the problems?  Without z-scoring, the 0 to 1 kJ/mol bin is out of control.  Replacing this with the `Inf` bin would also make the writeup considerably easier (lumping together all of the repulsive region into a single bin).
  * Based on the LASSO column in Figs/Testing/20180127_no_z_or_b0_regu_coef.tex, LASSO gives an excellent result when we don't consider the y-intercept, either.  But the ToBaCCo fit is terrible without the intercept (can't capture high-performing MOFs in the model), so bring that back.
* Recalculate the beta robustness figure (cached as an Rds file)
* Also consider setting the model intercept to zero (note: causes bad uptake values for ToBaCCo and some hMOFs.  The model can't fit the penalties correctly for the repulsive region, so there's a poor fit.)

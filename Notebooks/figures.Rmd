...
# For final submission, they like 600 dpi TIFF files.  Before then, let's use smaller figs of 300 dpi.
default_dpi <- 600
...
plot_grid(
  ...
  save_ben_fig("fig2.png", nrow = 2, base_aspect_ratio = 2.0)
...
# Save MOF-5 base_hist and betas for AIChE 2018 presentation
# Adapted theming from replot_hist_and_beta
aiche18_hist <-
  base_hist +
  scale_y_continuous(breaks=NULL, limits=c(0, 0.5), sec.axis = sec_axis(~., name="Volume fraction", breaks=c(0, 0.5))) +
  theme(axis.text.y.right = element_text(color="#8A226A"), axis.title.y.right = element_text(color="#8A226A"))
aiche18_hist
aiche18_hist %>% save_ben_fig("aiche18_hist.png", base_aspect_ratio = 2)
p_beta_orig %>% save_ben_fig("aiche18_beta.png", base_aspect_ratio = 2)
...
plot_grid(
  ...
  save_ben_fig("fig3.png", nrow = 3, base_aspect_ratio = 2)
...
hmof_partitioned_mod$plots %$%
  ...
  save_ben_fig("fig4.png", nrow = 2)
{hmof_partitioned_mod$plots$parity_training + xlab("GCMC capacity (g/L)") + ylab("Fitted capacity (g/L)")}%>% save_ben_fig("aiche18_training.png")
{hmof_partitioned_mod$plots$parity_testing  + xlab("GCMC capacity (g/L)") + ylab("Predicted capacity (g/L)")}%>% save_ben_fig("aiche18_testing.png")
...
hmof_partitioned_mod$plots %$%
  ...
  save_ben_fig("poster_row_fig4.png", ncol = 2)
...
p_tob_mod$plots %$%
  ...
  save_ben_fig("fig5.png", nrow = 2, ncol = 2)

#{p_ccdc_parity  + xlab("GCMC capacity (g/L)") + ylab("Predicted capacity (g/L)")}%>% save_ben_fig("aiche18_csd.png")
ccdc_h2_grids %>% 
  filter(id %in% ccdc_gcmc$id) %>% 
  # removing the db_name label
  eval_test_grid(mixed_partitioned_mod$trained_mod, ., default_binspec, mutate(ccdc_gcmc, y_act=g.L), db_name = "") %>% 
  {.$plots$parity_testing + xlab("GCMC capacity (g/L)") + ylab("Predicted capacity (g/L)")} %>% 
  save_ben_fig("aiche18_csd.png")
...
save_ben_fig(p_robust, "fig7.png", base_aspect_ratio=1.3)
p_robust
...
p_ch4_vol$plots %$%
  plot_grid(
    ...
    rel_heights=c(1.0/1.1, 1.0/2.0),
    align = "v",
    labels=default_subfig_labels, label_size=12, hjust=0, vjust=1
    ) %>% 
  save_plot(
    file.path(default_fig_dir, "fig6.png"),
    .,
    base_width = 8.3 / 2.54,
    ...
    nrow = 1.0/1.1 + 0.5,  # Need to set the total height, not the actual number of rows here
    dpi = default_dpi,
  )

{p_ch4_vol$plots$parity_testing %>% rescale_ch4_parity() + theme(axis.title.y = element_text(hjust=0.5)) + xlab("GCMC capacity (cm\u00B3/cm\u00B3)") + ylab("Predicted capacity (cm\u00B3/cm\u00B3)")}%>% save_ben_fig("aiche18_ch4.png")
...
for (arun_code in top_25_for_arun) {
  ccdc_h2_grids %>%
    ...
    {plot_hist_bins(., default_binspec) +
      scale_y_continuous("Density of sites", limits=c(0, 0.5)) +
...
save_ben_fig(p_mfu_isotherm, "sq_poster_fig8.png", base_aspect_ratio=1.75)
#plot_grid(
#  ggdraw() + draw_image("Figs/mfu_viz_rotated.png", scale=0.9),
#  p_mfu_isotherm,
#  ncol=2,
#  rel_widths = c(0.4, 1)
#  #labels=default_subfig_labels, label_size=12, hjust=0, vjust=1
#  ) %>%
#  save_ben_fig("lr_poster_fig8.png", ncol = 2, base_aspect_ratio = 1)
plot_grid(
...
plot_grid(
  ...
  save_ben_fig("fig8.png", nrow = 2, base_aspect_ratio = 1.75)
...
plot_grid(
  hmof_from_mixed$plots$test_ranking,
  tob_from_mixed$plots$test_ranking,
  ncol=2,
  labels=default_subfig_labels, label_size=12, hjust=0, vjust=1
  ) %>% 
...
# Repeat, including the Mixed coefficients
# Modify the p_coef_compare assignment earlier in this block so we can also export the intercepts
list(
    `hMOF` = hmof_partitioned_mod$trained_mod$mod,
    `ToBaCCo` = p_tob_mod$trained_mod$mod,
    `Mixed` = mixed_partitioned_mod$trained_mod$mod
  ...
  left_join(bin_loc_from_spec(default_binspec), by="bin") %>% 
  arrange(loc) %>% 
  select(bin, lower, upper, hMOF, ToBaCCo, Mixed) %>% 
  write_tsv("Figs/coef_mixed_export.tsv")
...
p_fine_diff_hist <- 
  ...
  geom_histogram() + 
  xlab("Difference in predictions from coarse input grid (g/L)")
...
p_fine_parity <- 
  fine_diff_df %$%
  parity_plot(fine05, coarse10) +
  xlab("Prediction from 0.5 Angstrom grid (g/L)") +
  ylab("Prediction from 1.0 Angstrom grid (g/L)")
...
plot_grid(
  ...
  save_si_fig("coarse_grid_points.png", ncol = 3)
...
```{r fine_hist_diff}
# Make a quantitative comparison between the 0.5~\AA and 1.0~\AA grid histograms, per reviewer 2 comments
r2_fine_grids <- hmof_fine_hist_sets$training
r2_coarse_grids <- hmof_h2_grid %>% filter(id %in% r2_fine_grids$id)

# Before we can compare the two counts, we need to transform the large matrix into histogram space.
# To get the requisite functions, we can step through `run_model_on_partitions` to see it eventually calls
# `eval_test_grid(align_bins="strict")` and `pred_grid`.  Within that function, the relevant lines of code are:
#  grid_desc <- test_grid %>%
#    stepped_hist_spec(binspec, align_bins = align_bins) %>% 
#    spread(key=bin, value=metric)

r2_fine_bins <- stepped_hist_spec(r2_fine_grids, default_binspec, align_bins="strict")
r2_coarse_bins <- stepped_hist_spec(r2_coarse_grids, default_binspec, align_bins="strict")

r2_bin_diff <- 
  rename(r2_fine_bins, fine="metric") %>% 
  inner_join(rename(r2_coarse_bins, coarse="metric"), by=c("bin", "id")) %>% 
  mutate(diff = coarse - fine, pdiff = diff / fine * 100) %>% 
  left_join(coef_tbl(hmof_partitioned_mod$trained_mod$mod), by="bin") %>% 
  mutate(dcdiff = diff * beta) %>% 
  left_join(bin_loc_from_spec(default_binspec), by="bin")

# Use absolute difference instead of percent for two reasons:
# 1. Contribution to deliverable capacity is directly based on the height of the bin
# 2. Percent is highly sensitive to poorly populated bins
r2_plot_bin_diff <- 
  r2_bin_diff %>% 
  ###ggplot(aes(group=bin, y=diff)) +
  #mutate(mylabel = factor(lower)) %>% 
  #ggplot(aes(x=mylabel, y=diff)) +
  # Adding in the bin locations:
  ggplot(aes(group=bin, x=loc, y=diff)) +
  #ggplot(aes(bin, diff)) +
  geom_boxplot() +
  #ylab("Difference in unit cell proportion (coarse - fine)") +
  ylab("Change in energy histogram bin") +
  xlab("Energy (kJ/mol)") +
  scale_y_continuous(breaks = 0.02*(-2:2))
print(r2_plot_bin_diff)
#save_si_fig(r2_plot_bin_diff, "r2_bin_diff.png")

# Merging in the coefficients of the mixed model to make a boxplot by difference in the predicted
# deliverable capacity.  If it's a small effect individually, then it really shouldn't matter.
r2_plot_dc_diff <- 
  r2_bin_diff %>% 
  ggplot(aes(group=bin, x=loc, y=dcdiff)) +
  geom_boxplot() +
  #ylab("Difference in predicted deliverable capacity due to bin") +
  ylab("Change in pred. deliverable capacity (g/L)") +
  xlab("Energy (kJ/mol)")
#save_si_fig(r2_plot_dc_diff, "r2_dc_diff.png")
r2_id_high_dc_diff <- 
  r2_bin_diff %>% 
  mutate(adiff = abs(dcdiff)) %>% 
  group_by(id) %>% 
  summarize(max_dcdiff = max(dcdiff)) %>% 
  filter(max_dcdiff > 0.5) %>% 
  .$id
...
plot_grid(
  ...
  save_si_fig("r2_plot_diff.png", ncol = 2, nrow = 1)


# In this next figure, I was trying to see if an increase in one bin was offset by 
r2_bin_diff %>% 
  filter(id %in% r2_id_high_dc_diff) %>% 
  left_join(gcmc_data, by="id") %>% 
  ggplot(aes(bin, dcdiff, group=id, color=lcd)) + geom_line() +
  scale_color_viridis()


# hmof_partitioned_mod$trained_mod
# But what about an enhancement in model accuracy? (Ilja's question about the directionality of the model predictions)
# The answer is again we're most interested in a rapid model that can directionally lead us to the right MOF subset, not quantitatively predicting the deliverable capacity
...
# From definition of p_ccdc_random -> p_ccdc_parity -> fig5.png
export_testing_data(ccdc_h2_grids %>% filter(id %in% ccdc_gcmc$id), rename(ccdc_gcmc, y_act=g.L), "Figs/ExportedData/ccdc_validation.csv", default_binspec)

...
p_ccdc_random$pred_df %>% mutate(core = (id %in% str_replace(core2_names, "_.*", "_stripped"))) %>% ggplot(aes(y_act, y_pred)) + geom_point() + parity_line + facet_wrap(~core) + coord_fixed()
p_tob_mod$plots %$%
  plot_grid(
    parity_training,
    parity_testing,
...
### General
* Use a combined hMOF/ToBaCCo model for training since the energy range of hMOFs is not sufficient (see `p_hmof_from_tob`): `gcmc_data %>% mutate(y_act = va100bar - va2bar) %>% eval_test_grid(p_tob_mod$trained_mod, hmof_hist_sets$testing, default_binspec, .)`.  Likely we're getting the strange split in data due to the formerly excluded low bins (or something with the z-scoring), hence the (relatively) poor transferability in ToBaCCo.

### More simulations/grids
* Get revised CCDC simulations from Scotty
...
### Textural properties
* Calculate density for the hMOFs (and consider recalculating all of the textural properties in Zeo++): get results from quest:/projects/p20067/BenBucior/EnergyGrid/PoreChar
...
* How do the range of the hMOF and ToBaCCo energies compare?  Is the maximally attractive ToBaCCo region significantly lower than the hMOFs? (related: does our ToBaCCo energy go low enough?)
  * TODO: start here, since it will potentially influence all of the other analysis.
  * `grids_h2 %>% filter(lower == -20.00 & counts >0) %>% View` to see if there's any outlier MOFs: there is one for ToBaCCo: tobmof5885.  Might not be necessarily physical, but worth putting the validation in place, anyway
  * On the bright side, this type of error doesn't show up in the CCDC energies.
  * Need to add a warning (with the filename) in the aggregation function.  Also consider changing the granularity to be coarser, which will improve all of the compile times and file sizes
* Removing the "hyperparameter tuning."  Also consider training with a fixed number and the rest as testing, if you pass a fixed value instead of a range.

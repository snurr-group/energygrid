...
### General
* Clean up notebook (old analysis, other paper, etc.)
* Resubmit new top CCDC MOFs
...
* If our model is solely based on LJ interactions, shouldn't the non-CNPS structures perform just as well?  Yes.  Add some analysis to the SI on non-CNPS, dimensionality, etc., to see if there's a new dependence. (based on prior analysis that's now being rerun, there isn't)
  * Also note that now the CCDC MOFs can accurately predict hMOFs, ToBaCCo, etc.
...
### z-scoring
* Recalculate the beta robustness figure (cached as an Rds file.  Should figure out if missing energy bins are caused by the new code, hMOF pecularity, or other)
* Adjust scaling factor for betas.  Possibly remove +/- red/blue color scheme for simplicity.
* Update manuscript to remove references to the z-scoring.  It adds a lot of complexity that we don't need.

...
And low pressure only, for hMOFs on ToBaCCo:

```{r low_p_tob}
temp_low_tob_pred <- tobacco_data %>%
  mutate(y_act = h2.g.L.2.77) %>%
  filter(!(is.na(y_act)) | y_act < 0) %>% 
  eval_test_grid(p_lo_model$fitted_model[[1]], tob_hist_sets$testing, default_binspec, .)
print(temp_low_tob_pred)

#temp_low_tob_pred$pred_df %>% 
#  select(id, y_act, y_pred, y_err) %>% 
#  rename(y_act_2 = y_act, y_pred_2 = y_pred, y_err_2 = y_err) %>% 
#  left_join(tob_deliv_from_hmof$pred_df, by="id") %>% 
#  ggplot(aes(y_err_2, y_err)) + geom_point()
# Note: tob_deliv_from_hmof is defined below
```
...
Note that while the Kendall rank correlation coefficient tau is only 0.5 vs. the 0.7 reported in `ahmed_balancing_2017`, Table S4, those data are solely based on the 35 bar data and not deliverable capacity.  Chahine's rule only seems to work at high uptake, not low pressure (thus it's not useful for delivarable capacity).

```{r vs_chahine}
gcmc_data %>% 
  select(surf.area..vol., starts_with("va")) %>% 
  gather(pressure, capacity, c("va2bar", "va100bar")) %>% 
  ggplot(aes(surf.area..vol., capacity)) +
  geom_point() +
  facet_wrap(~pressure)
```
...
```{r tobacco_by_node_type}
library(plotly)
(tob_deliv_from_hmof$pred_df %>% 
  mutate(same_nodes = (n1.name == n2.name)) %>% 
  mutate(node_info = str_c(id, topology, "\n", n1.name, n2.name, cbb.ID, sep=", ")) %>% 
  ggplot(aes(y_act, y_pred, text=node_info)) +
  geom_point() + 
  parity_line + facet_wrap(~same_nodes) + 
  coord_cartesian(xlim=c(0,60),ylim=c(0,60))
  ) %>% ggplotly #%>% highlight(on="plotly_selected")  # This may be a useful resource: https://plotly-book.cpsievert.me/linking-views-without-shiny.html
```
...
The top candidate, YUCFOS, is interesting because after processing it's just copper atoms distributed in space.  From spot checking, many of the top candidate materials are like this, probably because of the solvent and disorder removal steps.  A GCMC simulation will either show that it's possible to far surpass liquid density of 71 g/L with a hypothetical arrangement of atoms, or a huge error in regression model.  For YUCFOS, the GCMC has an error about unit cells (?) but low loading of 5.60+-0.51 cm3/cm3 absolute and high at 267.78+-15.09, which translates to a deliverable capacity of 23.4 g/L.  The large overestimation is likely overemphasis on void space from the model.

Going back to the unit cell point, RASPA calculates that based on cx/cy/cz in warnings.c:100?

## Testing a different binspec
```{r new_binspec}
# Based on some informal analysis, running the model without an intercept (and without normalization) still works awesome for the hMOFs, though it still doesn't translate to ToBaCCo.
# And manually dropping the lowest terms doesn't do much either.
# But bin 21 (0 to 1 kJ/mol) appears to have some difference in the model fits, so what if we play with this region?
binspec_shrink <- default_binspec
binspec_shrink["to"] <- 0
binspec_expand <- default_binspec
binspec_expand["to"] <- 5  # the maximum we pre-calculated in the original Rds files

shrink_h <- run_model_on_partitions(hmof_hist_sets, hmof_y_to_join, binspec_shrink, 0)
expand_h <- run_model_on_partitions(hmof_hist_sets, hmof_y_to_join, binspec_expand, 0)
print(shrink_h)
print(expand_h)

run_model_on_partitions(tob_hist_sets, tob_y_to_join, binspec_shrink, 0)
run_model_on_partitions(tob_hist_sets, tob_y_to_join, binspec_expand, 0)
tobacco_data %>% 
  mutate(y_act = h2.deliv.77) %>% 
  eval_test_grid(shrink_h$trained_mod, tob_hist_sets$testing, binspec_shrink, .)
tobacco_data %>% 
  mutate(y_act = h2.deliv.77) %>% 
  eval_test_grid(expand_h$trained_mod, tob_hist_sets$testing, binspec_expand, .)
```

### Adjusting lambda
Let's see if the diversity on training ToBaCCo failed just because of lambda:
```{r adjust_lambda}
tobacco_data %>% mutate(g.L=h2.deliv.77) %>% run_bin_model(diverse_results_tob$partitioned_hists$training, ., default_binspec["step"], default_binspec["width"], default_binspec[c("from", "to")], alpha = 0, lambda=1e-6) -> testing_diverse_low_lambda

tobacco_data %>% mutate(y_act = h2.deliv.77) %>% eval_test_grid(testing_diverse_low_lambda$fitted_model[[1]], diverse_results_tob$partitioned_hists$testing, default_binspec, .)

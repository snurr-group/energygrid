...
### CCDC applicability
TODO: redo this analysis with revised data on the full database.  Then make a list of the top candidates for Scotty's GCMC.
...
# First we need to obtain the revised data for 2 bar, 77 K, with the proper unit cells.
```
...
p_ch4_vol <- p_2bar_data %>% 
  ...
  run_model_on_partitions(p_ch4_sets, ., default_binspec, 0)
print(p_ch4_vol)
...
ch4_base_hist <- p_ch4_grids %>% 
  ...
  plot_hist_bins(default_binspec)
ch4_base_hist %>% overlay_cat_betas(coef_tbl(p_ch4_vol$trained_mod$mod), default_binspec)
...
list(`5.8 bar` = "ch4.cm3.cm3.5_8.298", `65 bar` = "ch4.cm3.cm3.65.298") %>% 
  ...
  map(function(x) run_model_on_partitions(p_ch4_sets, x, default_binspec, 0)) %>% 
  map(function(x) x$trained_mod$mod) %>% 
  map_dfr(coef_tbl, .id="cat") %>% 
  overlay_cat_betas(base_hist, ., default_binspec, scaling = 20.0)
# TODO: expand lower limits for CH4, perhaps to -40 (step 2) or lower, until the bottom bins are completely unpopulated?
...
TODO: top CCDC MOFs, re-implement predictions in C++?

### Data that still needs a final version
* ToBaCCo grids (with a script to verify line count)
* ToBaCCo 2 bar, 77 K data, with correct number of unit cells for RASPA (thus replacing 6 bar for deliverable capacity)
* Revised grids on the CCDC MOFs?  Full hMOFs?  Discuss plan with Scotty.

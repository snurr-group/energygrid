...
base_tob_deliv_p +
  facet_grid(n1.character ~ n2.character)
base_tob_deliv_p +
...
# Finally, how well does the model rank the materials?
tob_deliv_from_hmof$pred_df %>% 
  select(y_act, y_pred) %>% 
  cor(method = "spearman")
...
hmof_deliv_from_hmof <- hmof_y_to_join %>% 
  rename(y_act = g.L) %>% 
  eval_test_grid(trained_mod, hmof_hist_sets$testing, default_binspec, .)

hmof_deliv_from_hmof$pred_df %>% 
  select(y_act, y_pred) %>% 
  cor(method = "spearman")
hmof_deliv_from_hmof$pred_df %>% 
  mutate(r_act = rank(y_act), r_pred = rank(y_pred)) %>% 
...
TODO: see [caret documentation](http://topepo.github.io/caret/data-splitting.html#splitting-based-on-the-predictors) and other resources on how to perform this sampling.

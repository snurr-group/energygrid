...
# What about the ToBaCCo model on the ToBaCCo MOFs?
temp_tob_part_mod$pred_df %>% 
  select(y_act, y_pred) %>% 
  cor(method = "spearman")
temp_tob_part_mod$pred_df %>% 
  mutate(r_act = rank(y_act), r_pred = rank(y_pred)) %>% 
  ggplot(aes(r_act, r_pred)) +
  geom_point()
# TODO: consider adding the ranking analysis to the main partitioning analysis object
```
...
```{r ccdc_predictions}
pred_grid(trained_mod, ccdc_h2_grids, default_binspec) %>% .$y_pred %>% hist()
...
* Test a hybrid hMOF/ToBaCCo model on both databases.  Is it more general than the two separate models?
...
* CCDC: test predictivity on subset and suggest the top candiates
